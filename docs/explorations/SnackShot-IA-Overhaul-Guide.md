...
## I1. 결정 상세 화면 `/decision/[id]` (최우선 — 스키마 무변경) ✅ 완료(2026-07-14)

> 검토 개선(2026-07-14): 미결(deliberating) 결정에서는 ✎ 수정 버튼 숨김 — updateUserEdit이 status='edited'를 무조건 세팅해 미결이 confirmed_at 없는 확정으로 오염되던 결함(보드 이탈·마감 알림 취소). 미결의 내용 정리는 "결정했다" 전이 시트가 담당.
...
## I3. 투자 탭 + 종목 상세 (vNEXT 마이그레이션 1건) ✅ 완료(2026-07-14)
...
## I2. 5탭 재편 + 설정 이동 (스키마 무변경) ✅ 완료(2026-07-14)

> 검토 개선(2026-07-14): inbox의 확정 안내가 Alert 선택 없이 옵시디언을 무조건 열던 회귀 수정 — '닫기/열기' Alert 복원(DecisionBoard·결정 상세와 동일 UX, 무단 앱 전환 금지).
