  ...
  const header = (
    <View style={styles.headerContent}>
      ...
      <View style={styles.selectRow}>
        <Pressable onPress={toggleSelectAll} hitSlop={spacing.sm} style={styles.selectAllBtn}>
          <Ionicons
            name={allFilteredSelected ? 'checkbox' : 'square-outline'}
            size={iconSize.md}
            color={allFilteredSelected ? colors.brand.primary : colors.text.secondary}
          />
          <AppText preset="bodySmall" color={colors.text.secondary}>
            {allFilteredSelected ? '모두 해제' : '모두 선택'} ({filtered.length})
          </AppText>
        </Pressable>
        {selectedCount > 0 && (
          <Pressable onPress={clearSelection} hitSlop={spacing.sm}>
            <AppText preset="caption" color={colors.text.link}>{selectedCount}개 선택 · 해제</AppText>
          </Pressable>
        )}
      </View>
  ...
  return (
    <ScreenBackground edges={['top']}>
      <View style={styles.header}>
        ...
        <AppText preset="titleMedium" style={styles.headerTitle}>파일 관리</AppText>
      ...
      <FlatList
        ...
        ListEmptyComponent={<AppText preset="bodySmall" color={colors.text.tertiary}>해당하는 파일이 없어요</AppText>}
...
function FilterChips(
  ...
  return (
    <View style={styles.chips}>
      {options.map((o) => (
        <Pressable key={o.key} onPress={() => onSelect(o.key)} style={[styles.chip, o.active && styles.chipActive]}>
          <AppText preset="bodySmall" color={o.active ? colors.brand.onPrimary : colors.text.secondary}>{o.label}</AppText>
        </Pressable>
      ))}
    </View>
  );
